...
serve(async (req) => {
  ...
  try {
    const { query, childProfile, count = 2, startIndex = 0 } = await req.json();
    const language = childProfile.language || 'English';

    // Construct prompt for Claude to generate partial content blocks in a specific format and language
    ...
    These blocks should be the next set of blocks starting from index ${startIndex} (0-based).
    ...
    - These blocks are part of a larger set that is being generated progressively, so make them diverse but coherent.
