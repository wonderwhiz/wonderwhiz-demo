...
const VoiceInputButton: React.FC<VoiceInputButtonProps> = ({
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      recognitionRef.current.onerror = (event: any) => {
        ...
        if (event.error === 'no-speech') {
          toast.info(childAge < 8 ? "I didn't hear anything. Try again!" : "No speech detected. Please try again.");
        }
        stopListening();
      };
  ...
  const startListening = () => {
    if (recognitionRef.current) {
      try {
        ...
        setIsListening(true);
        toast.info(childAge < 8 ? "I'm listening! Tell me what you want to learn!" : "Listening... Speak your question clearly.");
      } catch (error) {
  ...
  return <div className="fixed right-6 bottom-6 z-30">
      <AnimatePresence>
        {isListening && transcript && <motion.div initial={{
        opacity: 0,
        y: 10,
        scale: 0.8
      }} animate={{
