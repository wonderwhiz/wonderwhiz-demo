...
export const ChildTaskList = ({
    ...
    try {
      const {
        ...
        error
          ...
          *,
          task:tasks (*)
        `).eq('child_profile_id', childId).eq('status', 'pending').order('assigned_at', {
        ascending: false
      });
  ...
  return <div className="space-y-3">
      {tasks.map(task => <div key={task.id} className="p-3 rounded-lg shadow-md border border-gray-100 bg-purple-200">
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <h3 className="font-medium">{task.task.title}</h3>
              {task.task.description && <p className="text-gray-500 text-sm mt-1">{task.task.description}</p>}
            ...
            <Button size="sm" variant="outline" className="border-wonderwhiz-purple text-wonderwhiz-purple hover:bg-wonderwhiz-purple/10" onClick={() => handleCompleteTask(task.id)}>
